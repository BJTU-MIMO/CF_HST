function [SE] = Compute_SE_MF_Scalable_Fractional(R,Q,hmean,I,ID,K,L,M,p,eta,sigma2,D)
...
for k = 1:K
    for s = 1:M
        term1(k,s) = sqrt(p*eta(k))*a(:,k,s)'*b(:,k);
        for i = 1:K
            for m = 1:M
                term2(k,i,s,m) = p*eta(i)*a(:,k,s)'*T(:,:,k,i,s,m)*a(:,k,s);
                if m == s
                   term3(k,s,m) = 0;
                else
                   term3(k,s,m) = sqrt(p*eta(k))*a(:,k,s)'*c(:,k,s,m);
                end
                if i == k
                   term4(k,i,s,m) = 0;
                else
                   term4(k,i,s,m) = sqrt(p*eta(i))*a(:,k,s)'*d(:,k,i,s,m);
                end
            end
        end
...
DS = abs(term1(:,:)).^2;
IT1 = squeeze(sum(sum(real(term2(:,:,:,:)),4),2));
IT2 = squeeze(sum(abs(term3(:,:,:)).^2,3));
IT3 = squeeze(sum(sum(abs(term4(:,:,:,:)).^2,4),2));
