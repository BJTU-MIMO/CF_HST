function [a_weight] = functionGetWeight(R,Q,hmean,I,ID,K,L,M,p,sigma2,D)
...
for k = 1:K
    for s = 1:M
        ...
        bb = b(:,k);
        xx = find(bb);
        G = squeeze(sum(p*sum(T(:,:,k,:,s,:),6),4)) + squeeze(p*sum(term1(:,:,k,s,:),5)) + squeeze(sum(p*sum(term2(:,:,k,:,s,:),6),4)) + sigma2*A(:,:,k);
        %G = G3(:,:,k,s);
        bb(all(bb==0,2),:)=[];
        G(find(all(G==0,2)),:)=[];
        G(:,find(all(G==0,1))) = [];
        ...
        for tt = 1:length(xx)
            a_weight(k,xx(tt),s) = a_vector(tt);
        end
