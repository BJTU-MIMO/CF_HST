...
SE_n_LSFD = zeros(K,M,Number,length(d_ve));

for n = 1:Number
    disp(['n------------------- ' num2str(n) ' out of ' num2str(Number)]);
    for sp = 1:length(d_ve)
        disp(['s----------- ' num2str(sp) ' out of ' num2str(length(d_ve))]);
        ...
        for k = 1:K
            for l = 1:L
                ...
                d(k,l) = (sqrt(dho(k,l)^2+d_ve(sp)^2));
...
plot(d_ve,squeeze(mean(LSFD(ss,:),1)),'b-o','LineWidth',2);
